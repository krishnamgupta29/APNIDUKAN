    ...
    return (
        <motion.div ref={ref} key="product-modal-root" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20 }}
                ...
                <button type="button" onClick={() => { console.log(typeof onClose); if (typeof onClose === 'function') onClose(); }} className="absolute top-4 right-4 z-50 p-2 bg-gray-100 rounded-full hover:bg-gray-200 transition"><X size={20}/></button>
                ...
                >
                    ...
                    {total > 1 && (
                        <>
                            <button type="button" onClick={()=>setCurImg((c)=>(c - 1 + total) % total)} className="absolute left-4 w-10 h-10 bg-white/80 backdrop-blur border border-gray-200 rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition hover:bg-white hover:scale-110 hidden md:flex z-10"><ChevronLeft/></button>
                            <button type="button" onClick={()=>setCurImg((c)=>(c + 1) % total)} className="absolute right-4 w-10 h-10 bg-white/80 backdrop-blur border border-gray-200 rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition hover:bg-white hover:scale-110 hidden md:flex z-10"><ChevronRight/></button>
                ...
                <div className="flex-1 p-8 md:p-12 flex flex-col justify-center border-l border-gray-100 overflow-y-auto">
                    ...
                    <button 
                        ...
                        onClick={(e)=>{e.preventDefault(); e.stopPropagation(); console.log(typeof onAdd, typeof onClose); if (typeof onAdd === 'function') onAdd(); if (typeof onClose === 'function') onClose();}} 
